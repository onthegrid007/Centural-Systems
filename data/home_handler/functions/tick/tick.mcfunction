...
execute as @p[tag=run_teleport_home] positioned 0 2 0 at @e[nbt={Item:{tag:{home_state:0L}}},limit=1,distance=2..] run tp @s[tag=homeRun] ~ ~ ~
execute as @p[tag=run_teleport_home] at @e[nbt={Item:{tag:{home_state:0L}}},limit=1] if entity @s[distance=..2] run tag @s remove homeRun
