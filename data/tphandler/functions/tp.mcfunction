...
execute as @e[nbt={Item:{tag:{tpState:0L}}}] store result entity @s Item.tag.requesterID long 1 run scoreboard players get @e[tag=TPRun,limit=1] TP_ID
...
execute as @e[scores={TP_Calc=0},limit=1] run function tphandler:sendrequest
execute unless entity @e[scores={TP_Calc=0},limit=1] as @e[nbt={Item:{tag:{tpState:0L}}},limit=1] run function tphandler:playerdoesnotexist
