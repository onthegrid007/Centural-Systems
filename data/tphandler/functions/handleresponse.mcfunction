...
execute if entity @e[tag=TPResponse,scores={TPReply=1..},limit=1] as @e[nbt={Item:{tag:{tpState:2L}}}] run function tphandler:tpaccept
execute if entity @e[tag=TPResponse,scores={TPReply=..-1},limit=1] as @e[nbt={Item:{tag:{tpState:2L}}}] run function tphandler:tpdeny
